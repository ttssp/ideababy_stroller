...
### 想法 (必填)
我的投资顾问会在微信小程序中发布每天的盘前盘后分析，偶尔会有关键的盘中分析预警，一周会有几次内部的直播视频回放，我要开发一个程序自动检测并收集这些内容，沉淀成我的个人消息/知识库/数据库，后续会用于投资决策

### 我为什么想做这个
我的投资顾问发布的信息很有用，但我没有太多时间实时follow，因此我要构建一个收集这些信息的自动化程序

### 我已经想过的角度
之前用proxyman去监听过，我发现过段时间token就过期了，每次都需要我登录账号，有点麻烦。我在想有没有更加自动化的方案，尽量最小化我的操作。

---

## **009**: 投资决策闭环(集合体 · 008采集 + 回测验证 + 004纪律)
...
### 已 build 可复用资产(查实于 XenoDev projects/004-pB)
004 已 ship 出"信号引擎"半个骨架(被承诺壳哲学包着、压在 v1.0 未释放):
- `strategy/base.py` StrategyModule 协议 IDL(source_id+analyze→StrategySignal,锁死"多信号源不合并")
- `xgboost_model.py` + `feature_engineering.py` + `train.py`(真 XGBoost 信号 lane)
- `correlation_audit.py`(信号源相关性<0.5 去重)
- `advisor_strategy.py` + domain/advisor.py(分析师内容已建模成带 structured_json 的信号源)
- 13 个 alembic migration(完整 DB schema)
008 = 闭环输入端(图文/回放/预警采集,v0.1/v0.2 已 ship + forge v4 定了信号 spike 前置)。
