...
echo ""
echo "=== codex round 4 finding #2 force-push 全变体 (must DENY)==="
run_case "--force-with-lease + main" deny "git push --force-with-lease origin main"
run_case "--force-with-lease 在 main 后" deny "git push origin main --force-with-lease"
run_case "--force-with-lease=ref 形式" deny "git push --force-with-lease=main:abc origin main"
run_case "--force-if-includes + master" deny "git push --force-if-includes origin master"
run_case "+main:main refspec" deny "git push origin +main:main"
run_case "+master refspec 简写" deny "git push origin +master"
run_case "+release-v1 refspec" deny "git push origin +release-v1"
run_case "+production refspec" deny "git push origin +production:production"
