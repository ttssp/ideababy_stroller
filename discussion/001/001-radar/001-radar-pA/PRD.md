...
**Version**: 1.9  (human-approved revision)
...
**Revised (v1.8)**: 2026-08-11 · **P4.1 首次真跑反馈**(hand-back `001-radar-pA-20260811T093217Z` · HANDBACK-LOG 第 46 条决议 · operator 三项裁决):首次真跑工程侧全绿(2 方向 exit 4 · 34/37 秒 · 1172 pytest 过 · coverage 99%)**而 operator 无法据以作出任何方向判断** —— 一手证据 = 两条 journal 行 `rationale` 逐字相同「没法判断,报告的格式、组织语言太随意,混杂太多符号、编号,编程语言等,人类无法理解」(绑 run_id + 快照)。**新增 O6**(产物可读 + 断言可核 · 三条判据全部锚定到 2026-08-11 两份真实产物,验收方式 = 判据必须能拒掉它们)+ **O1 修订**(结论词表补 `无法判断` 态、该态不计入 O1 计数、每条强制产 hand-back)。**P4.1 剩余真跑暂停**,等图与语料落盘(XenoDev spec 侧)+ 词表修订落地。
**Revised (v1.9)**: 2026-08-13 · **consumer 侧独立复跑发现 + O1 口径裁定**(hand-back `001-radar-pA-20260812T063139Z` · HANDBACK-LOG 第 48 条决议 · operator 四项裁决):v1.8 两个硬前置(缺陷 6 代码侧 + O6 判据校准)确已落地且 P4.1 首次真评估确已发生,**不推翻**。本轮改两处:**O6(a) 判据形态**——转义集由**枚举五字符**改为 **ASCII 标点转义类**(v1.8 的枚举写法漏 `\>`,而已计入 O1 的正对照产物含 3 处 `\>` 却 PASS);**O6 验收方式**——补正对照条款「必须 PASS **且零转义泄漏残留**」(v1.8 只写负对照必须 FAIL,该形态结构上无法证伪「判据欠包含」)。另落 **O1 计数口径裁定**(2026-08-11 两条「读不懂」行剔除 ⇒ **O1 = 2/5**,剩余 3 次)。
...
- **O6(v1.8 新增)· 产物可读 + 断言可核** —— per hand-back `001-radar-pA-20260811T093217Z` · HANDBACK-LOG 第 46 条

  > **为什么必须新增**:2026-08-11 P4.1 首次真跑,**1172 pytest 全过 · coverage 99% · 所有自动化 gate 全绿**,
  > 而 operator 读不了产物、无法作出任何方向判断。⇒ **既有验收体系里没有任何一条在检查「人能不能用」。**
  > 这一维不补,缺陷修完后换个 LLM 输出格式还会再犯。

  **🔴 三条判据全部锚定到 2026-08-11 的两份真实产物** —— 判据的验收方式是
  **「它必须能拒掉这两份」**。如此不需凭空发明阈值,且判据一定可失败(不是形容词)。
  锚定标的:`out/briefing/on-policy-distillation-20260811T085534Z.md` ·
  `out/briefing/coding-llms-20260811T085609Z.md`。

  **🔴 验收方式(v1.9 修订)· 负对照 + 正对照双侧,缺一不可**:

  | | 标的 | 判据 |
  |---|---|---|
  | **① 负对照** | 上述两份 2026-08-11 产物 | 必须 **FAIL**。通过即视为判据无效。 |
  | **② 正对照(v1.9 新增)** | 修复后的真实评估产物 | 必须 **PASS 且零转义泄漏残留**。 |

  > **为什么必须补 ②**:v1.8 只写了 ①。而「必须 PASS」型正对照**结构上无法证伪判据欠包含** ——
  > 一份**带着同类缺陷通过**的产物与一份**干净通过**的产物,在该形态下不可区分。
  > **实证**(HANDBACK-LOG 第 48 条 · consumer 侧独立复跑):已被 operator 采纳并计入 O1 的
  > `agentic-rl-for-llms-20260812T051207Z.md` 在扫描区内含 **3 处 `\>` 泄漏**却判 PASS,
  > 而负对照两份各含 92/84 次 —— **①只证明了极端处有区分力,没证明边界处有**。
  > ⇒ 通用形态记入本仓判据可达性族:**凡「正样本必须通过」型验收,都须同时带独立的缺陷缺席断言。**

  - **(a) 形式层 · 机器可判(v1.9 修订判据形态)**:落盘 briefing **不得含未消毒的 markdown
    转义泄漏**,且不得含超长无换行块。

    🔴 **转义泄漏的判据是「类」不是「清单」(v1.9)**:在扫描区(位移叙事 + 判断 claim)内,
    **任一 ASCII 标点字符前紧跟奇数条连续反斜杠**即判泄漏 —— 该二区的渲染路径按设计
    **不产生任何合法转义**,故区内出现转义只可能是「结构被压扁前」的旧行为复发。
    **不得回退为枚举字符清单**:v1.8 的五字符枚举(`\*`/`` \` ``/`\#`/`\|`/`\_`)已被实证漏掉 `\>`,
    枚举形态的覆盖率永远未知,**逐个补漏不是修法**。

    **验收锚 ①(负对照)**:该检查对上述两份产物必须 **FAIL**(实测泄漏 `\*` 92 次 /
    `` \` `` 84 次 / `\|` 20 次 / `\#` 6 次;单块 2117 与 2265 字符)。检查通过即视为判据无效。
    **验收锚 ②(正对照 · v1.9 新增)**:类判据落地后,`agentic-rl-for-llms-20260812T051207Z.md`
    **必须由 PASS 转 FAIL** —— 这是新判据在边界处确有区分力的证明。
    ⚠ **该产物已记入 journal,其 O1 计数不因本次判据收紧而撤销**(判据变更不追溯既有 O5/O7 样本;
    与 v1.6 O4「per-run 义务非全局豁免」同向:收紧只对其后的 run 生效)。
    ⚠ 修法方向承 v1.7 Biggest product risk 第二条:**上下文安全的展示投影,不是反转义**;
    证不出安全则展示版不 ship。**原件不得改动**(绑 journal `--snapshot-file`,改动破坏审计链)。
  - **(b) 效用层 · 用 O1 的 journal 度量**:briefing 中**每条结构性断言**
    (引用图 / 切片 / 关系 / 语料规模的陈述)必须可回溯到一份**落盘产物**;
    不可回溯的断言**不得出现在 briefing 中**。
    **验收锚**:上述两份产物必须 **FAIL** —— 其核心段落通篇描述方法关系图
    (`slice-1`/`slice-3` · 「11 个 `extends` 节点围绕 `OPD` 的辐条式拓扑」),
    而**图与检索语料均不落盘**,读者无法核验任何一条。
  - **(c) 结论层 · 见 O1 修订**:`无法判断` 结论的出现即为本条失效的直接信号。

- **O1 修订(v1.8)· 结论词表补「无法判断」态,且该态不计入 O1** —— 同上出处

  **现状缺陷**:结论词表 `("投","观望","不投")` 无「无法判断」。2026-08-11 operator 的真实状态是
  「读不懂 briefing,判断不了」,只能填 `观望` ⇒ **`观望`(已形成判断:决定再等)与
  「无法判断」(未形成任何判断)被折成同一个值**。

  🔴 **后果是 O1 可被虚假满足**:若 5 次全是「读不懂 → `观望`」,计数显示 **5/5 达标,
  而真正的方向评估一次都没发生**。⇒ 这与本仓 forge 006 v10 在审的失败族同形
  (比较 XD-41「缺席同时充当不适用与违规」):**两个不同状态折成一个值,系统总选看起来正常的那个解释。**

  **修订**:
  1. 结论词表**增加 `无法判断`** 态;
  2. **O1 的「≥5 次真实新方向评估」计数排除 `无法判断` 的行** —— 该态不是评估样本,是缺陷证据;
  3. **每出现一条 `无法判断` 即强制产 hand-back** —— 把它变成必须被处置的信号,而不是静默累积的计数。
     (不阻断使用;只是不许它沉默。)

  ⚠ **`无法判断` 不是失败,是诚实。** 把它计入 O1 才是失败 ——
  那等于允许「测不了」伪装成「测过了」。

- **O1 计数口径裁定(v1.9)· 2026-08-11 两条「读不懂」行剔除 ⇒ O1 = 2/5** —— per HANDBACK-LOG 第 48 条

  v1.8 的词表修订**只对其后新写入的行生效**;2026-08-11 那两条行(`run_id=c208444e-…` /
  `run_id=e37c92d7-…`)写入时词表尚无 `无法判断` 态,`conclusion` 字面值是 `观望`,
  `rationale` 逐字为「没法判断…人类无法理解」。**它们是缺陷 6 的原始证据,不是评估样本。**

  **裁定**:两行**不计入** O1 的 `distinct_direction_count`。生效口径:

  ```
  剔除前 = {coding LLMs, on-policy distillation, agentic RL for LLMs} = 3/5
  剔除后 = {coding LLMs, agentic RL for LLMs}                        = 2/5   ← 生效值
  decision_delta_count = 1(2026-08-02 那条,未变)
  ```

  ⚠ **`on-policy distillation` 是这两行在 journal 中的唯一方向,剔除后该方向整个消失** ——
  故剔除的真实代价是 O1 由 3/5 退到 **2/5**、**剩余需 3 次**真评估(不是 2 次)。

  🔴 **实现口径:不改代码、不给 append-only journal 引入 amend/tombstone 语义。**
  `summarize_journal_progress` 保持纯机械(只认 `conclusion` 字面值),故**机器读数仍显示 3**。
  **机器读数与治理口径在此处刻意分叉,以本条为准。** 理由:为一次历史订正引入 amend 语义,
  等于给审计链开一个永久可写口,代价远高于收益。
  ⇒ 任何消费 O1 计数的地方(报告 / gate / hand-back §3 进度行)**必须引本条**,不得直接读机器值。
...
### v1.8 response(P4.1 首次真跑反馈 @2026-08-11 · hand-back `001-radar-pA-20260811T093217Z` · HANDBACK-LOG 第 46 条)

(**与 v1.3~v1.7 全都不同:本次不是 forge verdict,也不是 codex 复核,而是 operator 亲自真用产品后的反馈。**
这是 PRD 首次因「产品实际不可用」而修订,此前六次修的都是判据形态、解锁链、时间锚这类治理面。)

1. **新增 O6 · 产物可读 + 断言可核 → 采纳**。
   触发事实:1172 pytest 全过 · coverage 99% · 所有 gate 全绿,而 **operator 读不了产物**。
   ⇒ **既有验收体系里没有任何一条在检查「人能不能用」。**
   ⚠ **判据形态是本条最要紧的设计**:三条判据**全部锚定到 2026-08-11 的两份真实产物**,
   验收方式 = **「判据必须能拒掉它们」**。这样做的理由:
   「可读」若写成形容词(如"产物应易于理解")就是一条**没有执行边的规则** ——
   而那正是本仓 forge 006 v10 同日刚裁掉的整族毛病。**锚定到会失败的真实样本,判据才有牙。**

2. **O1 修订 · 结论词表补 `无法判断` 态 → 采纳**。
   `("投","观望","不投")` 无「无法判断」⇒ operator 真实状态「读不懂,判断不了」只能填 `观望`
   ⇒ **两个不同状态折成一个值**,且 **O1 的「≥5 次」可被虚假满足**
   (5 次全是「读不懂→观望」则显示 5/5 达标而真评估零次)。
   修订三点:词表加态 / O1 计数排除该态 / **每条 `无法判断` 强制产 hand-back**(不阻断,但不许沉默)。
   ⚠ **与 forge 006 v10 的 XD-41 同形**(「缺席同时充当不适用与违规」)——
   *两个状态折成一个值,系统总选看起来正常的那个解释*。

3. **P4.1 剩余三次真跑暂停 → operator 决议**。
   现状 2/5 方向 · 0/1 `decision_delta`,但按第 2 条那个「2」**不是真评估**。
   暂停理由:图不落盘使 operator 读不了报告,词表缺态使「读不了」在计数里与「评估过了」不可区分;
   两者叠加 ⇒ **再跑三次即可凑满 5/5 而一次真评估都没发生**。
   今日 2 条 journal **保留**(缺陷证据,非评估样本);是否从计数排除,待词表改定后按新语义重判。

**本次未处置 · 已显式路由(HANDBACK-LOG 第 46 条)**:

- **缺陷 2 · 图与检索语料不落盘**(本轮最严重 · 使 briefing 的断言读者无法核验)——
  属 XenoDev spec 侧,**operator 已裁为 P4.1 继续的前置条件**;落盘形态与保留策略由 XenoDev 侧定。
  ⇒ **O6(b) 的验收依赖本条落地**,PRD 侧只定判据不定实现。
- **缺陷 1 · briefing 渲染器**(换行丢失 + 转义泄漏)—— XenoDev build 侧独立小修,可先行。
- **缺陷 4/5 · `gen-handback.sh`**(`source_repo` 不走 `IDS_ROOT` / 不产 §6.3.1 三字段)——
  已由 forge 006 v10 的 `OB-006-v10-09`(KG-B18)与 `OB-006-v10-07`(KG-B16)承接,**不新开条目**;
  本次贡献是把 KG-B18 的根因**从语料反推升级为代码级复现**。
