...
**Version**: 1.8  (human-approved revision)
...
- **O6(v1.8 新增)· 产物可读 + 断言可核** —— per hand-back `001-radar-pA-20260811T093217Z` · HANDBACK-LOG 第 46 条
  ...
  **「它必须能拒掉这两份」**。如此不需凭空发明阈值,且判据一定可失败(不是形容词)。
  ...
  - **(a) 形式层 · 机器可判**:落盘 briefing **不得含未消毒的 markdown 转义泄漏**
    (`\*` / `` \` `` / `\#` / `\|` / `\_`),且不得含超长无换行块。
    **验收锚**:该检查对上述两份产物必须 **FAIL**(实测泄漏 `\*` 92 次 / `` \` `` 84 次 /
    `\|` 20 次 / `\#` 6 次;单块 2117 与 2265 字符)。检查通过即视为判据无效。
