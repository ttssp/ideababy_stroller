...
**underweight-10 · 🔴 事后订正:第 7 行的一条判据证据有误(cwd 错致的假事实)**
本轮 IDS 侧提供并写进 forge-config X-8 / P1-Opus §1-B2 / 本表第 7 行的
「**实查 `tests/contract/` 不存在**」**是错的** —— 该检查从 worktree **根目录**跑
`ls tests/contract/`,而真实路径是 `projects/001-radar-pA/tests/contract/`,**cwd 错了**。
订正后事实:该目录**存在**,已有 3 个文件(`test_ephemeral.py`=**T032** / `test_jsonl_records.py`=**FU-A** /
`test_repo_scripts_gates.py`=**codex 补跑 R1 F4/F6**),**但无一属于 T040 的 6 个交付**;
逐一实查 T040 那 6 个文件**全部缺失**,且 T040 任务书 Inputs 原本就写明「`test_ephemeral.py`(C9)
已在 T032,本 task 补齐其余」。
⇒ **裁决(T040 立即局部起跑)不变,证据句改正。**
⚠ **但这是一个真实的方法论教训,必须记**:①「零进展」这个结论**在错误证据下也成立**,
所以**没有任何一轮 forge 能靠结论正确性发现证据是假的** —— 双方 P1/P2/P3 四轮全程照单引用;
