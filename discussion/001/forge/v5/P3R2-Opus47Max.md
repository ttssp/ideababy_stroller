...
## 5. ⚠ R2 后置接受(2026-08-02T14:45Z · 写于对方 P3R2 落盘之后)

**协议无 P3R3,而对方 P3R2 是在我之后写的并逐项修订了我的 12 项 —— 我没有机会在流程内表态。本节补上,以免 synthesizer 把「我未回应」误读为「我不同意」。**

**我全额接受对方 P3R2 §1 对 12 项的修订,无保留。** 其中三条是打在我身上的实质缺陷,逐条认:

1. **item 6 的 `≥5 run` 是我拍的魔数**,无任何来源。对方删得对,改「首个**预声明 epoch** 关闭时池仍 <3 才起上游 task」——**依据从数字换成了事件**,且该事件本就在 item 2 里预先定义。⚠ 讽刺的是,**本轮 forge 的主题正是「判据的来源与可达性」,而我在施工单里自己造了一个无来源阈值** —— 登记为本轮第三处「自指实例」。
2. **item 3 的判据「不得推出两种实装」本身不可机器判。** 对方改成稳定 requirement ID + 唯一 expected outcome + 验证命令 1:1 引用同 ID,语义歧义交有资质者复核。**我给 Unambiguous 写了一条不 Verifiable 的判据** —— 这是九特征在我自己的施工单上的第一次实战检验,而我没通过。
3. **item 12 我用「不作证据用途」标签解释掉了展示面的注入风险。** 对方指出标签不降低攻击面,改为「从规范存档生成**上下文安全的展示投影**;恶意样本在目标上下文必须 **inert**;**证不出安全则展示版不 ship**」。这条正中反自欺条款 —— 我在 P2 §3-5 才登记过自己一处「被解释掉」,**在同一轮的施工单里又犯了一次**。

另接受:item 1(warn 候选补 `evidence_plan` / `review_by` / 升 block 条件;**更早的可证伪点 = 过 `review_by` 后无证据仍被续期**,比我那条「几乎总是证不出」更早也更可测)· item 2(epoch/rubric 限定 + `closes_on` 不得事后延长 + 规范化 claim 去重 + anchor 抽样时仍须有效 + **当前 run 至少 1 条非豁免带证据判断才可借池**,全豁免 run 一律 `INSUFFICIENT_EVIDENCE` 不得借旧池空过 —— 这正是我请它攻击的「洞从单 run 换到多 run」,它给出了防线)· item 4(`start=min(first_journal_date, effective_date+30d)`,截止 = `start+3 months`;**我把 longstop 的 30 日与 3 个月观察窗混写了**)· item 8(versioned baseline + 须证「存量只 warn / 新增才 block」,防「名为 ratchet 实为全量门」)· item 10(`review_by < expires_on`;**到期转红是风险接受到期的预期语义,不是缺陷**;NIST 无固定 N,提前量由 operator review SLA 写入条目;实现超 S 只可人工预警,**机器转红不可省**)· item 11(裸词扫描限规范性 spec,历史引文不计)。

**接受对方新增的落地前置**:**R5 逐条裁决**(accepted / rejected / deferred + 映射到 12 项)。这是我漏的一处**证据闭环** —— 12 项里 item 10/11 直接落在 R5-F2(journal `lines[2]`)与 R4-F4 相邻的代码面上,而 **R5 四条修法至今未经 codex 复核、IDS 也从未逐条裁决**(决议 37 只裁了「第 6 轮何时跑」,没裁 R5 findings 本身)。⇒ 若不先裁,12 项中有若干条**建立在未裁决的地基上**。

**接受保留栏补三条**:`selfcheck` 正向 allowlist · C9 门② `DECLARED_READ_SITES` 声明机制 · C″ 分流契约 —— 明写「保留」以防落地时被误改。

**⇒ 最终 12 项以对方 P3R2 §1 表格为准,不以我 §4 那版为准。** 二者冲突处一律取对方版本。
