...
last_updated: 2026-08-13T08:20:00Z
total_decisions: 49
...
## 2026-08-11T13:25:09Z · 001-radar-pA · 本批 2 包合并决议(FU-HB46-1 / FU-HB46-2)

**第 47 条**。**类型**:hand-back 决议(2 包合并 —— 两包同为第 46 条缺陷 1 / 缺陷 2 的修复,
且它们**共同**决定「P4.1 能否解阻」这一个问题,拆两条会把该判断切碎)+ **consumer 侧横切发现**。

**Reviewed at**: 2026-08-11T13:25:09Z
**Handback packages**(按 ts 序):

| # | handback_id | task | 修的缺陷 | severity | validator |
|---|---|---|---|---|---|
| ① | `001-radar-pA-20260811T113300Z` | `FU-HB46-1` | 缺陷 1 · briefing 渲染器「结构安全投影」 | high | 6 约束 + check-7/8 全 PASS |
| ② | `001-radar-pA-20260811T125137Z` | `FU-HB46-2` | 缺陷 2 · 方法关系图 + 检索语料落盘 | high | 6 约束 + check-7/8 全 PASS |

两包均无 `ids_verdict_evidence:` 父键 ⇒ 按 R-Q6 跳过语法预检(见 (4),**本次性质与前几次不同**)。

**Operator decisions**:
- [x] **P4.1 维持暂停** —— 解阻前置从「缺陷 2」修正为「缺陷 2 + 缺陷 6 代码侧」(见 (1))
- [x] 修 XenoDev spec(跨仓)—— 缺陷 6 三点修订落代码 + 验收协议(见 (5))
- [x] **验收真跑用新方向**;旧两方向仅作 O6 判据校准、不进 journal(见 (5))
- [x] **O6 判据校准设为硬前置** —— 校准不过不开真跑(见 (5))
- [ ] 修 PRD(**显式不做** —— v1.8 的 O6 + O1 修订已覆盖,本条的顺序约束是从既有条款推出的)
- [ ] 修 SHARED-CONTRACT(**显式不做** —— `ids_verdict_evidence` 适用范围已是 forge 006 **阻断级**
      议题 (c),现在改 = 抢在 forge 前下结论,与决议 40/43 同项处理自洽)

---

### (1) 🔴 P4.1 解阻条件并未齐 —— 两包 §5 的「两个前置条件均已落地」是不完整的读法

两包 §5 均称「两个前置条件均已落地,P4.1 是否恢复由 operator 决定」。该读法只取了第 46 条 §(2)
(缺陷 2 列为前置),**漏了第 46 条 Operator decisions 原文**:「暂停 P4.1 剩余三次真跑,等
**缺陷 2 + 6** 修完」。consumer 侧实测缺陷 6 代码侧状态:

- IDS 侧 ✅ PRD v1.8 已定判据(词表加态 / O1 计数排除该态 / 每条强制产 hand-back)
- XenoDev 侧 ❌ **未落地** —— `projects/001-radar-pA/src/radar/journal/cli.py:65` 仍是
  `CANONICAL_CONCLUSIONS = ("投", "观望", "不投")`

⚠ **形态记录**:第 46 条诊断出的失败形态(两个状态折成一个值),在**解除该条暂停的这一步**原样复现 ——
「前置条件齐了」的判断本身就是把「两条前置」折成了「一条前置」。**修复者读自己修的那条,漏掉并列的另一条**,
是本仓「看起来是绿的」的又一实例。

### (2) ⭐ 维持暂停的**真正**理由(本条对第 46 条的推理升级)

第 46 条给的理由是「再跑三次即可凑满 5/5 而一次真评估都没发生」(计数被虚假满足)。本条给出更根本的理由:

> 验收真跑只有两种结果:**可读** → operator 形成真实判断;**仍不可读** → 形成不了判断。
> 哪种会发生,跑之前不知道 —— 这正是它有信息量的原因。而当前词表**只能表达第一种**。
> ⇒ **一次只有一种结果可被记录的实验,不产生信息。**

该理由强于计数理由,且解释了为什么**不能**用「事后人工重判」绕过去:事后重判要求在结果已知后回填状态,
而系统必须在结果发生的那一刻就能接住它。**缺陷 6 因此是验收真跑的前置,而非并行项。**

### (3) 🟡 缺陷 1「已修」目前**没有真跑产物**佐证

两份冻结产物(`out/briefing/*-20260811T0855*.md`)是修复**前**的 —— 两包 §5 声明不动,处理正确(绑
journal `--snapshot-file`,改动破坏审计链)。但 consumer 侧实测另有发现:

- `projects/001-radar-pA/out/briefing-readable/` 下有两份同名再渲染产物(本地 mtime 17:23,晚于
  `out/briefing/` 的 16:56,早于本包 created 11:33Z=19:33 本地 ⇒ **开发中途产物**)。
- **两包均未提及该目录**;仓内 `rg` 无任何代码引用该路径(`--glob '!out/**'` 零命中)。
- 其 narrative 段标题呈 `###`,而 shipped 代码把叙事来源标题钳到
  `_MIN_STRUCTURAL_HEADER_LEVEL = 4`(`src/radar/reconstruct/writer.py:108`)⇒ **该目录不是 shipped
  路径的输出**(强推断 · 待 XenoDev 确认出处)。

⇒ **不得用作缺陷 1 的验收证据。** 缺陷 1 目前的验收只有 33 条单测 + 手工合成样例 —— 而缺陷 1 本来是被
**一次真跑**定义出来的(journal 逐字「人类无法理解」)。**「推演≠真跑」在其自身的修复验收上复现。**

⚠ 该中间产物另呈两处残留:`### 总结:接合链条的三重结构位移| 位移维度 |…` 标题与表头粘同一行
⇒ 表格不渲染成表格。是否为 shipped 路径的残留缺陷,**须由真跑判**,本条不下结论。

**✅ 正面核实(据实记)**:两包 §4 主张的 red-green 证据链**实存** ——
`tests/red-green-log/FU-HB46-1-briefing-structural-safe.log` 与 `FU-HB46-2-graph-corpus-persist.log`
均在磁盘上。(consumer 首次核对时用 `tail -5` 截断误判为缺失,全量 `ls` 后推翻 —— 记为**核查方法本身
产生假阴性**的实例。)

### (4) 🟡 `ids_verdict_evidence` 缺席 —— **连续第 3、4 个**,且首次命中 forge 006 (c) 点名的类

⚠ **本条先更正 consumer 自身的初判**:该缺口**不是新发现**。决议 40 记为「攒批」,决议 43 已
**升级为 `/expert-forge 006` 阻断级议题 (c)**,措辞为:

> **(c)** consumer precheck **「缺席即豁免」** 必须反转为:**声称跑过 review 的包缺
> `ids_verdict_evidence` 块 ⇒ warn/reject**,不再静默跳过。

**本批两包是该措辞点名的类的第一批实例**:此前缺块的包各有正当豁免(非 codex verdict / `codex_model`
无法诚实填写 —— 见决议 40 (B) 认可的先例),而**本批两包各自明写跑了 5 轮 codex**(①
`adversarial-review`、② `review`),即**声称跑过 review 却无块**。consumer 侧再次全程无感、不 warn。

- 复发计数:决议 43 记「连续第 2 个」⇒ 本批后为 **连续第 4 个**。
- 相关实测:`review-log/` 下**无 FU-HB46 目录** ⇒ 10 轮 codex 的原始产物不在该约定位置
  (与决议 43 ③「raw log 只存 2/11 轮且在 git 之外」同族,本次为 **0/10**)。
- **本条不改 SHARED-CONTRACT** —— 归 forge 006 (c),等机制而非再加一条建议。

### (5) 验收协议(operator 两项裁决 · 跨仓下发)

**问题**:Q1「不恢复 P4.1」与 Q2「要一次真跑产物验收」字面冲突。**解法不是加状态位**(那等于在缺陷 6
之上再造一个同形缺陷:两种目的折进同一条 journal 记录),而是**按目的选方向**:

| | 用什么 | 进 journal? | 目的 |
|---|---|---|---|
| **① O6 判据校准**(硬前置) | 08-11 两份旧产物,过 shipped 路径重渲染 | 否 | 验 O6 三条判据能否**拒掉**已知坏产物 |
| **② 第一次真评估** | **一个全新方向** | 是 | 既验工具,也是 O1 的 1/5 |

- **① 是硬前置,校准不过不开 ②**。判据拒掉旧产物 ⇒ 有区分力,可用于验收新跑;判据放过旧产物 ⇒
  **判据是空的**,在花掉一次真跑之前就知道。成本低(无 LLM 调用)。这也是 PRD v1.8 O6「验收方式 =
  判据必须能拒掉它们」这一**证伪锚**的首次真执行 —— 此前它只是写着。
- **② 必须用新方向**:旧两方向已被 operator 在 hand-back、决议、中间产物中反复阅读,作为
  「工具是否改变了我的决策」的样本**已污染**。换新方向后,同一次跑天然同时是合法验收与干净样本,
  张力消失且**无需新机制**。结果可读 ⇒ 计入 O1(1/5);不可读 ⇒ 记 `无法判断`、不计入、强制产 hand-back。

**跨仓下发给 XenoDev 的内容**:
1. 缺陷 6 三点修订落代码(词表加 `无法判断` / O1 计数排除该态 / 每条强制产 hand-back);
2. ① 的 O6 判据校准,产出校准结论;
3. ② 的真跑走 **shipped 路径**,产 briefing + graph + corpus 三类产物;
4. 说明 `out/briefing-readable/` 出处,并清理或显式打标(防将来被误当证据)。

### (6) codex 门两包均未过 · operator 显式覆盖 · **披露诚实**

两包都明写「不得记为『codex 门已过』」,并如实列出每轮 finding 与修法。**与决议 40 的 XD-41
(「不应合入」被转述成「披露即可合」)不同型** —— 本次是诚实标注,不是转述漂移,据实记为正面先例。

**残余敞口(据实记)**:① 是 spec §6 明写的**注入防御边界**代码,round-5 仍找出**新类别**绕过
(括号式有序列表 `1)`),即 **finding 未收敛到零就停**。风险不是「已知有 bug」,而是
**不知道还剩多少同类 CommonMark 构造**(ATX/Setext × 两种有序列表标记 × 两种 fence 定界符…)。
② 最后 3 轮集中在 `atomic_publish.py` 的罕见文件系统错误路径(ENOSPC/EIO),收敛形态明显更好。
本条记录该覆盖决策,以便将来若真出绕过时可追溯到此处。
...
## 2026-08-13T08:20:00Z · 第 48 条载体更正 · PRD v1.8→v1.9(裁决 2 的落点从 build 改回 PRD)

**第 49 条**。**类型**:consumer 自身决议的**事后订正**(体例同第 8 行 `forge-001-v6-resample` 订正条)。
append-only 不改第 48 条正文,本条为其**权威更正**;读第 48 条必须连读本条。

**Reviewed at**: 2026-08-13T08:20:00Z
**更正对象**:第 48 条 Operator decisions 第 5 行 —— `[ ] 修 PRD(**本条不改**)`

### 🔴 更正内容:该栏勾错了

第 48 条 §(3) 把「补 O6a 转义表 + B3 判据升级」当作**跨仓下发给 XenoDev 的 build 侧修复**。
生成下发指令前实查 PRD 原文,发现**两项都是 PRD v1.8 的条款文本**,不是实现自选:

- `PRD.md:114`(v1.8 原文)**逐字枚举**了那五个字符:「不得含未消毒的 markdown 转义泄漏
  (`\*` / `` \` `` / `\#` / `\|` / `\_`)」⇒ `_ESCAPE_LEAK_CHARS` 五元组**不是实现漏了,
  是忠实实现了 PRD**;
- 同段「**验收锚**:该检查对上述两份产物必须 **FAIL**」也是 PRD 原文 ⇒
  「只测极端负例」的校准形态同样源于 PRD,不是 build 侧协议。

⇒ 按铁律「**PRD is source of truth after L3;L4 agents read PRD but don't alter product
decisions**」,XenoDev **不得**自行把判据集扩到 PRD 文本之外。**裁决 2 的载体是 PRD amendment,
不是 build 修 bug。** 若按第 48 条原文下发,会造出一段 **code-ahead-of-PRD 漂移窗口** ——
与本仓已在册的两份 spec 分叉、forge 施工单 stale 同族。

⚠ **形态记录**:第 48 条本身正是靠「consumer 不只读包、自己重跑」抓出 producer 的判据缺口;
而**该条自己的处置落点又是靠『生成下发指令前再实查一次原文』才纠正的**。
⇒ **「决议正确」与「决议的载体正确」是两件独立的事,前者不蕴含后者。** 记入本仓判据可达性族邻域。

**Operator decisions**(本条):
- [x] **修 PRD** —— v1.8→**v1.9**,三处修订(见下),operator 显式批准
- [x] **O1 口径写进 PRD** —— 不再只存于第 48 条散文脚注
- [ ] 修 SHARED-CONTRACT(不涉)
- [ ] 修 XenoDev spec(**本条不改**;XenoDev 按 v1.9 实现,不新增 spec 条款)

### PRD v1.9 三处修订(commit `c996dbb`)

1. **O6(a) 判据形态:枚举 → 类**。改为「扫描区(位移叙事 + 判断 claim)内**任一 ASCII 标点字符
   前紧跟奇数条连续反斜杠**即判泄漏」,并明写**不得回退为枚举清单**。理由:该二区渲染路径按设计
   不产生任何合法转义,故区内出现转义只可能是旧行为复发;**枚举形态的覆盖率永远未知,
   逐个补漏不是修法**。
2. **O6 验收方式:补正对照条款**。v1.8 只有「① 负对照必须 FAIL」;v1.9 加
   「② 正对照必须 **PASS 且零转义泄漏残留**」。新增**验收锚 ②**:类判据落地后
   `agentic-rl-for-llms-20260812T051207Z.md` **必须由 PASS 转 FAIL**(边界处区分力的证明)。
   ⚠ 同时写死:**该产物的 O1 计数不因判据收紧而撤销** —— 收紧不追溯既有 O5/O7 样本,
   与 v1.6 O4「per-run 义务非全局豁免」同向。
3. **O1 计数口径裁定入 PRD**。第 48 条裁决 1 的 `O1 = 2/5`、剩余 3 次、
   「不改代码不引入 amend 语义 ⇒ 机器读数仍显示 3,机器读数与治理口径刻意分叉」
   全部写进 PRD 正文,并加一句:**任何消费 O1 计数的地方必须引本条,不得直接读机器值**。
   理由:第 48 条把该口径只写在决议散文里,**靠人记得才不出错** —— 正是本仓反复踩的
   「只写在散文里的口径无人消费」(比较 009 KG-52 散文通道零覆盖)。

### 🟡 附带记录:跨仓 `O6` 编号撞车(不在任何包内 · 归 006)

- IDS `PRD.md:102` **O6 = 产物可读 + 断言可核**(v1.8 新增)
- XenoDev `specs/001-radar-pA/spec.md:271` **O6 = taste 反馈标记**(`usage: recurring`)

**同名两义**,跨仓下发里裸写「O6」有歧义。属已在册的「KG 编号空间对齐」族
(前例:`KG-32` 两义 / IDS HANDOFF 记 KG-31 而包记 KG-33)。**本次下发一律写全称
「IDS PRD v1.9 O6(a)」** 规避;机制修法归 `/expert-forge 006`。

---

**Follow-up commits**:`c996dbb`(PRD v1.8→v1.9)
