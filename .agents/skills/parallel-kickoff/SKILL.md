...
description: Orchestration patterns for running multiple Codex and Codex sessions in parallel via git worktrees. Load when planning or executing parallel task dispatch, when the user asks to "parallelize", "kick off workers", "fan out tasks", or "use worktrees".
...
Codex --worktree T003-auth-service
# In parallel terminal:
Codex --worktree T004-chat-service
# Codex side:
...
### Clean up when done
Codex auto-cleans empty worktrees. For manual management:
```bash
git worktree list
...
```bash
# See orphans
find .Codex/worktrees -maxdepth 1 -mindepth 1 -type d

# If Codex crashed mid-session, worktrees with uncommitted changes persist.
# Codex auto-cleans safe (clean, older than cleanupPeriodDays) ones at startup.
```

## Recommended concurrency by plan

| Subscription | Safe concurrent Codex sessions | Notes |
|---|---|---|
...
**Practical recommendation**: 3 Codex + 1 Codex + 1 Spark for mini-tasks = 5 total
