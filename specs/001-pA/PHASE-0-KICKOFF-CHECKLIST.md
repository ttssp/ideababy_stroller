...
- [ ] **T001 LLM spike fixture 采集计划**
  - 预计谁采：operator
  - 预计时长（独立估算）：3–6h（T001 总 8h 里的 fixture 收集部分）
  - 20 篇（或升到 40 篇？）× {shift, incremental, unrelated} 分布确认
  - 若 fixture 难采：可不可以用近 6 个月 lab 组会记录反推（你真看过的 paper，哪些让你改判过）
...
## §3 · 数据源调研（T001 fixture 前）

- [ ] **arXiv cs.AI + cs.LG + cs.CL 日更数量**：你了解过吗
  - 预期 100–300 篇/天 跨 3 个 category
  - 8–15 topic 每 topic 0–20 candidate/day
- [ ] **个人 lab 真实覆盖的 topic 列表**（8–15 个）：草稿在哪
...
- [ ] **已知的 10 个 state-shift 案例**（T001 fixture 用）：至少能举 5 个
  - 例：Mamba 替换 attention · DPO 替 RLHF · GPT-4 技术报告 · test-time compute 崛起 · Diffusion 模型路径 · ...

---

## §4 · 技术决策锁定（Phase 0 前必须确定）

- [ ] **LLM primary provider**: Anthropic (推荐) / OpenAI
  - T001 spike 完后会确定。现在只需 env 都备好，实际决策在 T001 报告里
...
## §6 · Phase 0 执行顺序（小心关键路径）

**绝对优先级**：

1. **T001 LLM spike** (独立，8h · 可完全独立开工，不等任何东西)
   - blocks: T004, T010, T011
   - 必须产出 `projects/001-pA/spikes/T001-llm-provider-report.md` 含 `approved-provider: <name>`
   - 若失败 → `fallback-to-heuristic` 路径 → T012 按 spec §4.1 启发式
2. **T002 monorepo scaffold** (4h · 不阻塞 T001，可并行)
...
4. **T004 LLM interface** (5h · 等 T001 + T002) - Opus 级 task
   - cp `llm-types.ts` + `llm-*-stub.ts` + `llm-utils.ts` → `src/lib/llm/`
   - 按 `reference/llm-adapter-skeleton.md §3/§4` 填 TODO
   - **必做**: F2 confidence downgrade (§3 末段) · F1 truncateTo3Sentences import · F3 adversarial fixtures 15 case 齐

**顺着跑完 Phase 0 的其余 task**（T005 arXiv / T006 auth / T007 env / T008 test harness），然后进 Phase 1。
...
- [ ] **接受 T001 spike 可能 fail**：有 20% 概率两个 provider 都 < 70% 准确率，这时必须 fallback 到纯启发式 —— **不意味着项目失败**，是成本可控选择
...
- [ ] `git commit -m "chore(001-pA): freeze spec v0.2.2 before Phase 0 kickoff

Spec package passes R_final adversarial review (verdict: <CLEAN/CLEAN-WITH-NOTES/CONCERNS>).
Starting Phase 0 with T001 + T002 parallel.

Co-Authored-By: Claude Opus 4.7 (1M context) <noreply@anthropic.com>"`
- [ ] `git tag spec/001-pA/v0.2.2`
...
| 症状 | 首选排查 | SOP |
|---|---|---|
| T001 两家 provider 都 < 70% | 延长 fixture 到 40 篇 / 加 prompt 迭代 2 轮 | 若仍 < 70% → 接受 fallback heuristic，升 `tasks/T012` 为主路径，T004 adapter 只做 summarize（judge 不走 LLM） |
...
| Anthropic API 403 | key 错 / 地域问题 | 换 OpenAI 跑 T001；T004 interface hot-swap |
