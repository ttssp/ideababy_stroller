...
**版本**: 0.3.0
**创建**: 2026-04-23
**上次修订**: 2026-04-24（R_final BLOCK fix G1/G2/G3/G4 · §5 task ID 结构重写 · 见变更日志）
...
| **D15** | **Per-paper summary 持久化在独立 `paper_summaries` 表** per `(paper_id, topic_id, prompt_version)` · `llm_calls` 仅作审计表（不存 response body）· T013 写双表 · T014/T015/T016/T023 读此表 | 2026-04-23 R1 adversarial fix（Codex B1）· 解决 IN-7 原设计 summary 无权威落点的结构合同空洞 |
