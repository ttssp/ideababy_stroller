...
### 2.4 **两家共用的 LLMProvider interface**（T007 必须先落到 `src/lib/llm/provider.ts`）
...
/** R1 adversarial fix (B1) · summarize() 返回的富结构 · caller 持久化到 `paper_summaries` 表 */
...
export type StateShiftVerdict =
  | { kind: 'shift'; confidence: number; rationale: string }
  | { kind: 'incremental'; confidence: number; rationale: string }
  | { kind: 'unrelated'; confidence: number; rationale: string }

export interface LLMProvider {
  readonly name: string  // e.g. "anthropic" | "openai"

  /** ≤ 3 句话 summary；返回 SummaryRecord 富结构（R1 B1）· caller 负责写 llm_calls + paper_summaries */
  summarize(input: {
  ...
  /** 给定 candidate + earlier anchor + topic · 返回单对 verdict（per-pair）· caller 循环外部 batch */
  judgeRelation(input: {
    ...
    topic: { id: number; name: string; keywords: readonly string[] }
  }): Promise<StateShiftVerdict & { inputTokens: number; outputTokens: number; latencyMs: number }>
}
```

**契约**：
- 两家 adapter 返回结构完全一致；**spike 后切换 provider = 改 env + restart worker**，不改业务代码
- **casing 分层**：接口层 camelCase（上述 `summaryText` / `promptVersion` / `modelName`）；DB 列 snake_case（`paper_summaries.summary_text` / `.prompt_version` / `.model_name`）；`src/lib/summary/persist.ts` 是两层之间的唯一映射点
...
- adapter 本身**不**写 `llm_calls` 或 `paper_summaries` 表；T013 `runSummaryPass` 的 caller 在事务内先 INSERT `llm_calls` 得 `llm_call_id`，再 INSERT `paper_summaries(paper_id, topic_id, summary_text, llm_call_id, model_name, prompt_version)`
